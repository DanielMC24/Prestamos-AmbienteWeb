  ...
  if (identificacion.length >= 9) {
    $.ajax({
      ...
      success: function (data) {
        /**Esto lo utlizo para poder referenciar y poder ver los datos en la pantalla mediante un ID en JS */
        if (data.resultcount > 0) {
          /**Esto lo utilizo para desabilitar el boton  */
...
function desactivar(idusuario) {
  bootbox.confirm("¿Está Seguro de desactivar el usuario?", function (result) {
    if (result) {
      $.post(
        "../ajax/usuarios.php?op=desactivar",
        { idusuario: idusuario },
        function (e) {
          bootbox.alert(e);
          tabla.ajax.reload();
